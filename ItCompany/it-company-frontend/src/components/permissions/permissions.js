...
function Permissions(props) {
  ...
  const numberOfCheckedEngineer = (items) =>
    intersection(checkedEngineer, items).length;
      ...
      <CardHeader
        avatar={
          <Checkbox
            ...
            checked={
              numberOfCheckedEngineer(items) === items.length &&
              items.length !== 0
            }
            indeterminate={
              numberOfCheckedEngineer(items) !== items.length &&
              numberOfCheckedEngineer(items) !== 0
            }
        ...
        {items.map((value) => {
          ...
          return (
            <ListItem
              ...
              role="listitem"
              button
              onClick={handleToggleEngineer(value)}
            >
              <ListItemIcon>
                <Checkbox
  ...
  const numberOfCheckedProjectManager = (items) =>
    intersection(checkedProjectManager, items).length;
  ...
  const handleCheckedRightProjectManager = () => {
    setRightProjectManager(
      rightProjectManager.concat(leftCheckedProjectManager)
    );
    setLeftProjectManager(not(leftProjectManager, leftCheckedProjectManager));
    setCheckedProjectManager(
      not(checkedProjectManager, leftCheckedProjectManager)
    );
  };
  const handleCheckedLeftProjectManager = () => {
    setLeftProjectManager(
      leftProjectManager.concat(rightCheckedProjectManager)
    );
    setRightProjectManager(
      not(rightProjectManager, rightCheckedProjectManager)
    );
    setCheckedProjectManager(
      not(checkedProjectManager, rightCheckedProjectManager)
    );
  };
      ...
      <CardHeader
        avatar={
          <Checkbox
            ...
            checked={
              numberOfCheckedProjectManager(items) === items.length &&
              items.length !== 0
            }
            indeterminate={
              numberOfCheckedProjectManager(items) !== items.length &&
              numberOfCheckedProjectManager(items) !== 0
            }
            disabled={items.length === 0}
            inputProps={{
              "aria-label": "all items selected",
        ...
        title={title}
        subheader={`${numberOfCheckedProjectManager(items)}/${
          items.length
        } selected`}
      />
        ...
        {items.map((value) => {
          ...
          return (
            <ListItem
              ...
              role="listitem"
              button
              onClick={handleToggleProjectManager(value)}
            >
              <ListItemIcon>
                <Checkbox
  ...
  const leftCheckedProjectManager = intersection(
    checkedProjectManager,
    leftProjectManager
  );
  const rightCheckedProjectManager = intersection(
    checkedProjectManager,
    rightProjectManager
  );
  ...
  const numberOfCheckedHrManager = (items) =>
    intersection(checkedHrManager, items).length;
      ...
      <CardHeader
        avatar={
          <Checkbox
            ...
            checked={
              numberOfCheckedHrManager(items) === items.length &&
              items.length !== 0
            }
            indeterminate={
              numberOfCheckedHrManager(items) !== items.length &&
              numberOfCheckedHrManager(items) !== 0
            }
            disabled={items.length === 0}
            inputProps={{
              "aria-label": "all items selected",
        ...
        title={title}
        subheader={`${numberOfCheckedHrManager(items)}/${
          items.length
        } selected`}
      />
        ...
        {items.map((value) => {
          ...
          return (
            <ListItem
              ...
              role="listitem"
              button
              onClick={handleToggleHrManager(value)}
            >
              <ListItemIcon>
                <Checkbox
  ...
  const getAndSetAllData = async () => {
    try {
      ...
      const res2 = await interceptor.get("/privilege/all/ROLE_EMPLOYEE");
      const names2 = res2.data.map((item) => item.name);
      setLeftEngineer(names2);
      setRightEngineer(
        names.filter((permission) => !names2.includes(permission))
      );

      const res3 = await interceptor.get("/privilege/all/ROLE_CLIENT");
      const names3 = res3.data.map((item) => item.name);
      setLeftHrManager(names3);
      setRightHrManager(
        names.filter((permission) => !names3.includes(permission))
      );
    } catch (e) {
      console.log(e);
    }
  ...
  const updateHrManager = () => {
    ...
    interceptor
      .post("privilege/update", sendData)
      .then((res) => {})
      .catch((err) => {
        console.log(err);
      });
          ...
          >
            <TabPanel value={value} index={0} dir={theme.direction}>
              <Grid container spacing={1}>
                <Grid item>
                  {customListEngineer("Has permissions ", leftEngineer)}
                </Grid>
                <Grid item>
                ...
                <Grid item>
                  {customListEngineer("Available permissions", rightEngineer)}
                </Grid>
              </Grid>
            ...
            <TabPanel value={value} index={1} dir={theme.direction}>
              <Grid container spacing={1}>
                <Grid item>
                  {customListHrManager("Has permissions ", leftHrManager)}
                </Grid>
                <Grid item>
                  <Grid container direction="column" alignItems="center">
                    <Button
                ...
                <Grid item>
                  {customListHrManager("Available permissions", rightHrManager)}
                </Grid>
              </Grid>
